...
def more_than_Length(seq_list,length):
    count =0
    list_more_than_length = []
    for seq in seq_list:
        if(len(seq) >= length ):
            count = count + 1
            list_more_than_length.append(seq)
    print("No. of sequences greater than  length  '{} ' are '{}' ".format(length,count))
    return list_more_than_length
...
def four_and_five(list,len,val):
    list_of_more_than_len_str=[]
    list_of_more_than_len_str = more_than_Length(list,len)
    GC_content_list = cal_GC_conntent(list_of_more_than_len_str)
    count = more_than_GC(GC_content_list, val)
    print("Combining results of  min_length & GC_score both.......")
    print("no. of Sequences  with length  more than {}  and GC_score   more  than  {} are only  {} ".format(len,val,count))
...
if(choice == '0'):
    ...
    GC = int(input())
    GC_content =  cal_GC_conntent(list)
    count_gc = more_than_GC(GC_content,GC)
    print("no.  of sequences with gc_score more than {}  are {}".format(GC,count_gc))
    if play   ==  True:
        print_choices(list)
